          ...
          script {
            dir("${repoFolder}") {
              sh """
                ...
                env        = sh(script: "jq '.global_config.environment' -r ${repoFolder}/manifest.json", returnStdout: true).trim()
                name       = sh(script: "jq '.global_config.name' -r ${repoFolder}/manifest.json", returnStdout: true).trim()
                region     = sh(script: "jq '.global_config.region' -r ${repoFolder}/manifest.json", returnStdout: true).trim()
                org        = sh(script: "jq '.global_config.organization' -r ${repoFolder}/manifest.json", returnStdout: true).trim()
                acme_email = sh(script: "jq '.cluster_config.certmanager_email_address' -r ${repoFolder}/manifest.json", returnStdout: true).trim()
