        ...
        steps {
          script {
            try {
              ...
              zip -r app.zip app
              """
              veracode applicationName: "${image}", timeout: 5, createProfile: true, criticality: "Medium", debug: true, waitForScan: true, scanName: "counter-service-build-${buildNumber}", uploadIncludesPattern: 'app.zip', vid: "${veracode_api_id}", vkey: "${veracode_api_key}"
              }
            }
            catch (Exception e) {
