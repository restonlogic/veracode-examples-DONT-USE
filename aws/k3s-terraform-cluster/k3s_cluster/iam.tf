resource "aws_iam_instance_profile" "ec2_instance_profile" {
  name = "${var.common_prefix}-ec2-instance-profile-${var.global_config.environment}"
  role = aws_iam_role.aws_ec2_custom_role.name

  tags = merge(
    local.global_tags,
    {
      "Name" = lower("${var.common_prefix}-ec2-instance-profile-${var.global_config.environment}")
    }
  )
}

resource "aws_iam_role" "aws_ec2_custom_role" {
  name = "${var.common_prefix}-ec2-custom-iam-role-${var.global_config.environment}"
...
resource "aws_iam_policy" "cluster_autoscaler" {
  name        = "${var.common_prefix}-cluster-autoscaler-policy-${var.global_config.environment}"
  path        = "/"
  ...
  tags = merge(
    local.global_tags,
    {
      "Name" = lower("${var.common_prefix}-cluster-autoscaler-policy-${var.global_config.environment}")
    }
  )
}

resource "aws_iam_policy" "aws_efs_csi_driver_policy" {
  name        = "${var.common_prefix}-csi-driver-policy-${var.global_config.environment}"
  path        = "/"
  ...
  tags = merge(
    local.global_tags,
    {
      "Name" = lower("${var.common_prefix}-csi-driver-policy-${var.global_config.environment}")
    }
  )
}
...
resource "aws_iam_policy" "allow_secrets_manager" {
  name        = "${var.common_prefix}-secrets-manager-policy-${var.global_config.environment}"
  path        = "/"
  ...
  tags = merge(
    local.global_tags,
    {
      "Name" = lower("${var.common_prefix}-secrets-manager-policy-${var.global_config.environment}")
    }
  )
}
