...
resource "aws_cloudwatch_event_rule" "ec2_spot_interruption_warn" {
  name        = "${var.common_prefix}-ec2-spot-interruption-warn-${var.global_config.environment}"
  ...
  tags = merge(
    local.global_tags,
    {
      "Name" = lower("${var.common_prefix}-ec2-spot-interruption-warn-${var.global_config.environment}")
    }
  )
}
...
resource "aws_sqs_queue" "ec2_spot_interruption_warn_queue" {
  name                      = "${var.common_prefix}-ec2-spot-interruption-warn-queue-${var.global_config.environment}"
  ...
  tags = merge(
    local.global_tags,
    {
      "Name" = lower("${var.common_prefix}-ec2-spot-interruption-warn-queue-${var.global_config.environment}")
    }
  )
}
...
resource "aws_cloudwatch_event_rule" "ec2_spot_request_fulfillment" {
  name        = "${var.common_prefix}-ec2-spot-fulfillment-${var.global_config.environment}"
  ...
  tags = merge(
    local.global_tags,
    {
      "Name" = lower("${var.common_prefix}-ec2-spot-fulfillment-${var.global_config.environment}")
    }
  )
}
...
resource "aws_sqs_queue" "ec2_spot_request_fulfillment_queue" {
  name                      = "${var.common_prefix}-ec2-spot-fulfillment-queue-${var.global_config.environment}"
  sqs_managed_sse_enabled   = true
  message_retention_seconds = 7200
  ...
  tags = merge(
    local.global_tags,
    {
      "Name" = lower("${var.common_prefix}-ec2-spot-fulfillment-queue-${var.global_config.environment}")
    }
  )
}
