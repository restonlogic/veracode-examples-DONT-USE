...
first_instance=$(aws ec2 describe-instances --filters Name=tag-value,Values=k3s-server-${environment} Name=instance-state-name,Values=running --query 'sort_by(Reservations[].Instances[], &LaunchTime)[0].[InstanceId]' --output text | head -n1)
